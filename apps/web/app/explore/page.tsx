...
function TokenCard({ token, rank }: { token: ExploreTokenItem; rank?: number }) {
  ...
  return (
    <Link href={href} className="premium-card group flex items-center gap-3 p-4 transition hover:-translate-y-0.5 hover:border-white/20">
      {rank != null && (
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-[11px] text-cyan-300 opacity-0 transition group-hover:opacity-100">
          Open token page
        </p>
      </div>
  ...
  return (
    <div className="premium-card overflow-hidden">
      ...
      <div className="flex items-center gap-3">
        <Link href={href} className="token-orb h-8 w-8 text-xs font-bold">
          {token.chainKey?.slice(0, 3).toUpperCase() ?? token.symbol.slice(0, 2)}
        </Link>
        <div className="min-w-0 flex-1">
          <Link href={href} className="block font-semibold text-white truncate hover:text-cyan-200">
            {token.name}
          </Link>
      ...
      <div className="flex flex-wrap gap-2">
        <Link href={href} className="rounded-full border border-cyan-400/30 bg-cyan-400/10 px-2 py-1 text-xs text-cyan-200">
          Token page
        </Link>
        {token.websiteUrl ? (
