...
- `apps/extension` — Manifest V3 browser extension shell with background/content/inpage/popup/options surfaces
...
- The provider bridge is now live for connect/accounts/chainId/switchChain plus common `window.ethereum` EVM compatibility methods; approved EVM sign/send requests can execute inside the extension after a second signer confirmation, while WalletConnect relay and broader non-EVM provider execution still remain gated
...
- Dapp capability placeholder

Target capabilities for the next expansion waves:

- Swap quote/execution contracts
- Dapp signing/transaction review contracts on top of the active bridge
