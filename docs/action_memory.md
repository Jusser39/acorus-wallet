...
248. Re-ran public production checks during live-activation prep: `https://24wallet.ru/health` returned `200`, `https://24wallet.ru/api/swap/evm/status` still returned `configured:false`, the public `price` smoke still returned `503`, `https://24wallet.ru/extension-smoke` returned `200`, and `https://bstcrm.ru/healthz` returned `200`.
...
260. Deployed the final API fix to `/opt/acorus-wallet-release-current`, rebuilt the wallet API, and rechecked public production endpoints without touching the separate CRM stack.
261. Final public/live smoke passed: `https://24wallet.ru/health` returned `200`, `/api/swap/evm/status` returned `configured:true`, public native ETH -> USDC `price` and `quote` returned `200`, public USDC -> WETH approval quote returned `200`, `https://24wallet.ru/api/market/prices?...` returned `200`, `https://24wallet.ru/extension-smoke` returned `200`, `https://bstcrm.ru/healthz` returned `200`, and `node scripts/smoke-zerox-live.mjs` reported `0x live smoke: PASS`.
...
274. Production checks passed after deploy: `https://24wallet.ru`, `/explore`, `/tokens/1/native?family=evm&symbol=ETH&name=Ethereum`, `/tokens/101/NATIVE?family=solana&symbol=SOL&name=Solana`, `/api/market/chart?...range=ALL`, and `/api/swap/evm/status` responded correctly; 0x status remained `configured:true`; `https://bstcrm.ru/healthz` returned `200` with GET.
