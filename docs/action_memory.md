...
529. Committed and pushed the production swap UI sync as `751b707e445a1fe0b313e03c1db07ccfa64791b8` with message `Fix swap picker UI and production shell`.
530. Deployed `751b707e445a1fe0b313e03c1db07ccfa64791b8` to `/opt/acorus-wallet-release-current`, preserving server backup `/root/backups/acorus-swap-picker-ui_20260527_024513`, then rebuilt/recreated the wallet API, web, and nginx containers.
531. Production curl checks passed for `https://24wallet.ru`, `/swap`, `/tokens/coingecko/bonk?source=coingecko&symbol=BONK&name=Bonk`, `/api/swap/evm/status`, and `/downloads/acorus-wallet-extension.zip`.
532. Production browser QA confirmed the stale swap labels are gone, the network selector is a flat custom list, the token picker opens inline inside the swap card, Solana shows expanded popular tokens, BONK shows a swap composer instead of `Route coming next`, and Wallet/Explore navigation works.
533. Production browser QA did not reproduce the Chrome/Brave freeze after the refreshed bundle: the page accepted clicks, route navigation worked, and token search typed normally. Existing browser instances may still need a hard reload or site-data clear if they cached an older service worker/bundle.
534. Added a PWA manifest icon polish by changing the web manifest from missing `/icon.png` to `/icon.svg` and adding `apps/web/public/icon.svg`; focused web test/build passed after the icon fix.
