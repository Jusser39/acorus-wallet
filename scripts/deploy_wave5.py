...
remote_tar = f"{REMOTE_PATH}/acorus-wave5-deploy.tar.gz"
...
run(f"cd {REMOTE_PATH} && tar xzf acorus-wave5-deploy.tar.gz --overwrite && rm acorus-wave5-deploy.tar.gz")
run(f"cd {REMOTE_PATH} && docker compose --env-file .env -f infra/docker-compose.yml build api web", timeout=600)
run(f"cd {REMOTE_PATH} && docker compose --env-file .env -f infra/docker-compose.yml up -d api web nginx")
time.sleep(5)
run(f"cd {REMOTE_PATH} && docker compose --env-file .env -f infra/docker-compose.yml exec -T api npx prisma db push --schema=prisma/schema.prisma", timeout=60)
time.sleep(3)

# Verify health
run("curl -fsS http://127.0.0.1:8080/health")
run("curl -fsS http://127.0.0.1:8080/api/chains")
run("curl -I -s http://127.0.0.1:8080/send | head -5")

# Verify persistence
run(f"cd {REMOTE_PATH} && docker compose --env-file .env -f infra/docker-compose.yml restart api")
time.sleep(8)
run("curl -fsS http://127.0.0.1:8080/health")

# Public URL
run("curl -fsS http://85.239.59.199:8080/health")
